...
public class MemberController {
	...
	public String login(MemberVO vo, LoginVO loginvo, Model model, HttpSession session, HttpServletResponse response)
			throws IOException {
		...
		if (user != null) {
			...
			return "event/event_main"; /* redirect:" + ( dest != null ? (String)dest : "/.do"); */
		} else {
...
//				return "redirect:/member/login.do";
	 ...
	 * public void join_company(MemberVO vo, HttpServletRequest request,
	 * HttpServletResponse response) throws IOException {
	 * 
	 * System.out.println(vo.toString());
	 * 
	 * 
	 * String[] phones = request.getParameterValues("phone1"); String phone = "";
	 * 
	 * String addr2 = request.getParameter("member_addr2");
	 * 
	 * for (String s : phones) {
	 * 
	 * phone += s; phone += "-"; } phone = phone.substring(0, phone.length() - 1);
	 * 
	 * vo.setMember_phone(phone);
	 * 
	 ...
	 * 
	 * int result = memberService.memberJoin(vo);
	 * response.setContentType("text/html; charset=euc-kr;"); PrintWriter pw =
	 * response.getWriter(); if (result > 0) { // 회원가입 성공 -> home으로 이동
	 * pw.println("<script>alert('회원가입 성공');location.href='" +
	 * request.getContextPath() + "'" + "</script>"); } else {
	 * pw.println("<script>alert('회원가입 실패');location.href='" +
	 * request.getContextPath() + "'" + "</script>"); } pw.flush();
	 * 
	 * 
	 * 
	 * }
	 ...
	 */
	/*
	 * @RequestMapping(value = "/join_kakao.do") public String join_kakao(MemberVO
	 * vo, Model model, HttpServletResponse response, HttpServletRequest request)
	 * throws IOException {
	 * 
	 * 
	 * MemberVO member = memberService.selectOne(vo);
	 * 
	 * if(member != null) { //카카오 회원으로 회원가입 이미 했을 경우 자동 로그인됨
	 * 
	 * 
	 * HttpSession session = request.getSession(true);
	 * 
	 * MemberVO login = new MemberVO(); login.setMember_idx(member.getMember_idx());
	 ...
	 * 
	 * 
	 * 세션 등록 session.setAttribute("login", login);
	 * 
	 * return "redirect:/.do";
	 * 
	 * }else { //카카오 회원으로 회원가입 한게 없을 경우
	 * 
	 * model.addAttribute("kakaoVo", vo); return "member/join_seller"; } }
	 */
